    ...
    res.format({
      html: () => {
        if (people.length > 0) {
          ...
          throw new Error('Pessoas não encontradas. Tente novamente de dia.')
        }
      },
      json: () => {
        if (people.length > 0) {
...
router.post('/', async (req, res) =>{
  if(!req.body.name)
  {
    req.flash('error', 'nome vazio não vai dar');
    res.redirect('back');
    return;
  }
try{
  const [result] = await db.execute('INSERT INTO person (id,name,alive) VALUES (NULL,?,TRUE);', [req.body.name]);
  if(result.affectedRows===1){
    req.flash('success', 'adicionou o cara')
  }
  else
  {
    req.flash('error','não deu pra adicionar');
  }
}
catch(e)
{
...
router.delete('/:id', async (req,res) => {
  try{
    ...
    if(result.affectedRows===1){
      req.flash('success', 'deletou o cara');
    }else{
      req.flash('error','cara nem existe no bd');
    }
  }
  catch(e)
  {
